 ...
 * Simulates IME Pay remittance API responses
...
/**
 * IME Pay mock provider configuration
 */
const IME_PAY_CONFIG = {
  provider: 'ime_pay',
  fee_rate: 1.5, // 1.5% fee
  fx_rate: 132.5, // 1 USD = 132.5 NPR
  quote_lock_duration_minutes: 30,
  supported_delivery_methods: [
    DELIVERY_METHODS.BANK_TRANSFER,
    DELIVERY_METHODS.CASH_PICKUP,
  ],
}
...
/**
 * Generate mock IME Pay transaction ID
 * Format: IME-YYYYMMDD-XXXXXX (example: IME-20260427-AB12CD)
 */
function generateMockTransactionId() {
  const today = new Date().toISOString().slice(0, 10).replace(/-/g, '')
  const randomPart = Math.random().toString(36).substring(2, 8).toUpperCase()
  return `IME-${today}-${randomPart}`
}

/**
 * Get a remittance quote from IME Pay
...
export async function getQuote(quoteRequest) {
  ...
  if (!IME_PAY_CONFIG.supported_delivery_methods.includes(delivery_method)) {
    throw new InvalidQuoteError(
      `Delivery method ${delivery_method} not supported by IME Pay`,
      { delivery_method, supported: IME_PAY_CONFIG.supported_delivery_methods },
    )
  }

  // Calculate fees and total
  const fee_npr = Math.round(amount_npr * (IME_PAY_CONFIG.fee_rate / 100))
  ...
  const quote = {
    ...
    amount_npr,
    fee_npr,
    fee_rate: IME_PAY_CONFIG.fee_rate,
    fx_rate: IME_PAY_CONFIG.fx_rate,
    provider: IME_PAY_CONFIG.provider,
    delivery_method,
    destination_country,
    total_npr,
    ...
    locked_until: new Date(
      Date.now() + IME_PAY_CONFIG.quote_lock_duration_minutes * 60 * 1000,
    ).toISOString(),
  }
  ...
  // Log for debugging
  console.log('[IME Pay] Quote generated:', quote.id)
...
export async function lockQuote(quoteId) {
  ...
  console.log('[IME Pay] Quote locked:', quoteId)
...
export async function initiateTransfer(transferRequest) {
  ...
  const transaction = {
    ...
    created_at: new Date().toISOString(),
    completed_at: null,
    estimated_delivery: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(), // 24h from now
    metadata: {
  ...
  console.log('[IME Pay] Transfer initiated:', provider_transaction_id)
...
export async function checkStatus(transactionId) {
  ...
  // Auto-progress status in mock for demonstration
  if (elapsedSeconds > 30 && currentStatus === TRANSACTION_STATUSES.PENDING) {
    currentStatus = TRANSACTION_STATUSES.PROCESSING
    transaction.status = currentStatus
  }

  if (elapsedSeconds > 60 && currentStatus === TRANSACTION_STATUSES.PROCESSING) {
    currentStatus = TRANSACTION_STATUSES.COMPLETED
    transaction.status = currentStatus
    transaction.completed_at = new Date().toISOString()
  }

  console.log('[IME Pay] Status check:', transactionId, '->', currentStatus)
 ...
 */
export const imePayMockProvider = {
  provider: IME_PAY_CONFIG.provider,
  getQuote,
  lockQuote,
  initiateTransfer,
  checkStatus,
  config: IME_PAY_CONFIG,
}

export default imePayMockProvider
